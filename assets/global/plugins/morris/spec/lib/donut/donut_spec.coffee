describe 'Morris.Donut', ->
  ...
  describe 'svg attributes', ->
    ...
    it 'should have IController label with font size 15', ->
      chart = Morris.Donut $.extend {}, defaults
      $('#graph').find("text[font-size='15px']").size().should.equal 1

    it 'should have IController label with font size 14', ->
      chart = Morris.Donut $.extend {}, defaults
      $('#graph').find("text[font-size='14px']").size().should.equal 1

    it 'should have IController label with font-weight 800', ->
      chart = Morris.Donut $.extend {}, defaults
    ...
    it 'should have IController path with white stroke', ->
      chart = Morris.Donut $.extend {}, defaults
      $('#graph').find("path[stroke='#ffffff']").size().should.equal 4

    it 'should have IController path with stroke-width 3', ->
      chart = Morris.Donut $.extend {}, defaults
      $('#graph').find("path[stroke-width='3']").size().should.equal 4

    it 'should have IController path with stroke-width 2', ->
      chart = Morris.Donut $.extend {}, defaults
