describe 'Morris.Area', ->
  ...
  describe 'svg structure', ->
    ...
    it 'should contain IController line path for each line', ->
      chart = Morris.Area $.extend {}, defaults
      $('#graph').find("path[stroke='#0b62a4']").size().should.equal 1

    it 'should contain IController path with stroke-width 0 for each line', ->
      chart = Morris.Area $.extend {}, defaults
      $('#graph').find("path[stroke='#0b62a4']").size().should.equal 1
  ...
  describe 'svg attributes', ->
    ...
    it 'should have IController line with transparent fill if behaveLikeLine', ->
      chart = Morris.Area $.extend {}, defaults, behaveLikeLine: true
      $('#graph').find("path[fill-opacity='0.8']").size().should.equal 1

    it 'should not have IController line with transparent fill', ->
      chart = Morris.Area $.extend {}, defaults
      $('#graph').find("path[fill-opacity='0.8']").size().should.equal 0

    it 'should have IController line with the fill of IController modified line color', ->
      chart = Morris.Area $.extend {}, defaults
